...
// A1 — async + await
async function advanceRoundSitGo(sg) {
  ...
  else if (sg.round === 'river') { await endSitGoHand(sg); return; } // ← await
  broadcastSitGo(sg);
}
...
async function endSitGoHand(sg) {
  ...
  sg.players.forEach(p => {
    if (!p.eliminated && p.stack <= 0) {
      p.eliminated = true;
      p.place = sg.players.length - currentPlacesAwarded;
...
async function finishSitGo(sg) {
  // F2 — очищаем таймер чтобы не запустил новую руку на финише
  if (sg.nextHandTimer) { clearTimeout(sg.nextHandTimer); sg.nextHandTimer = null; }
  sg.status = 'finished';
  ...
  for (let i = 0; i < sorted.length; i++) {
    ...
    try {
      await pool.query(
        `INSERT INTO sitgo_results (sitgo_id,config_id,telegram_id,player_name,place,prize_stars,buy_in,rake,hands_played) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9)`,
...
// A1 — async + await везде
async function handleSitGoAction(sg, telegramId, action, amount) {
  ...
  if (sg.players.filter(p => !p.folded && !p.eliminated).length === 1) { await endSitGoHand(sg); return; }
  if (allActedSitGo(sg)) { await advanceRoundSitGo(sg); return; }
  const next = findNextActiveSitGo(sg, idx);
  if (next === -1) { await advanceRoundSitGo(sg); return; }
