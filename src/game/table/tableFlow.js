...
function scheduleNextHand(t) {
  ...
  t.status='waiting'; // S3 — сразу сбрасываем статус чтобы checkAutoStart работал
...
async function endHand(t) {
  // S2 — guard от двойного вызова
  if (t.round==='showdown' || t.status==='finished') return;
  ...
  // A2 — сохраняем всех через Promise.allSettled, один упавший не блокирует остальных
    ...
    t.players
      ...
      .map(async p => {
        ...
        try {
          ...
          console.error('saveBalance error for', p.telegramId, e);
        }
      })
  );
